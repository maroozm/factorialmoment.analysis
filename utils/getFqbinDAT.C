...
void getFqbinDAT()
{
  ...
  //const char* wagonName[wagons] = { "fb128" };
  const Int_t wagons = 24;
  const char* wagonName[wagons] = { "fb128", "fb768", "fb128wcuts", "fb128Prim", "fb128womat", "fb128wowd", "fb128wcutsPrim", "fb128wcutswomat", "fb128wcutswowd", "fb768Prim", "fb768womat", "fb768wowd", "fb128i1", "fb128i2", "fb128wcutsi1", "fb128wcutsi2", "fb128i1i2", "fb128wcutsPrimi1", "fb128wcutsPrimi2", "fb768i1", "fb768i2", "fb768i1i2", "fb768Primi1", "fb768Primi2" };
  const Int_t bins = 2;
  ...
  for (Int_t nFiles = 0; nFiles < files; nFiles++) {
    ...
    for (Int_t nWagons = 0; nWagons < wagons; nWagons++) {
      for (Int_t nBins = 0; nBins < bins; nBins++) {
        ...
        TList* list1 = (TList*)file[nFiles]->Get(Form("ntplistRe%dtaskfm_cond%d", nBins + 1, nWagons + 1));
        if (!list1) {
          std::cout << "Error getting list " << Form("ntplistRe%dtaskfm_cond%d", nBins + 1, nWagons + 1) << std::endl;
          exit(1);
        } else {
          std::cout << " Reading list " << Form("ntplistRe%dtaskfm_cond%d", nBins + 1, nWagons + 1) << std::endl;
        }
         list1->ls();
        for (Int_t t = 0; t < Ntmax; t++) {
          TString fNtpname = Form("fntpMBinrecbin%s_%s_M%d", ptName[nBins], wagonName[nWagons], t);
          // fNtpname += t + 1;
